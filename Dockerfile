...
ADD  build/distributions/MicroV-Dropwizard-Microservice.tar  /
...
# add it explicitly
ADD appconfig.yml /MicroV-Dropwizard-Microservice/

# Convenience if we ever want to log into the image and snoop around
WORKDIR /MicroV-Dropwizard-Microservice
...
ENTRYPOINT ["/MicroV-Dropwizard-Microservice/bin/MicroV-Dropwizard-Microservice", "server", "appconfig.yml"]
